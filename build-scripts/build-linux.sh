...
#   - Git
#
# Usage: ./build-linux.sh [--clean] [--release-only] [--debug-only] [--openssl-path /path/to/openssl3]
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --openssl-path    Specify OpenSSL installation path (e.g., /opt/openssl3)"
            echo "  --help            Show this help"
            echo ""
            echo "Environment variables:"
            echo "  OPENSSL_CFLAGS    OpenSSL compiler flags (e.g., -I/path/to/openssl/include)"
            echo "  OPENSSL_LIBS      OpenSSL linker flags (e.g., -L/path/to/openssl/lib -lssl -lcrypto)"
            echo "  PKG_CONFIG_PATH   Path to OpenSSL .pc files"
            exit 0
            ;;
        *)
...
if [ -n "$OPENSSL_PATH" ]; then
    ...
    # Check for lib64 first (RHEL/CentOS), then lib
    if [ -d "$OPENSSL_PATH/lib64" ]; then
        export OPENSSL_LIBS="-L$OPENSSL_PATH/lib64 -lssl -lcrypto"
        export LD_LIBRARY_PATH="$OPENSSL_PATH/lib64:$LD_LIBRARY_PATH"
        if [ -d "$OPENSSL_PATH/lib64/pkgconfig" ]; then
            export PKG_CONFIG_PATH="$OPENSSL_PATH/lib64/pkgconfig:$PKG_CONFIG_PATH"
        fi
    elif [ -d "$OPENSSL_PATH/lib" ]; then
        export OPENSSL_LIBS="-L$OPENSSL_PATH/lib -lssl -lcrypto"
    ...
    if command -v "$OPENSSL_PATH/bin/openssl" &> /dev/null; then
        OPENSSL_VERSION=$("$OPENSSL_PATH/bin/openssl" version)
...
build_config() {
    ...
    # Copy shared library
    if [ -f "src/cslib/.libs/libcs.so" ]; then
        cp -P src/cslib/.libs/libcs.so* "$OUTPUT_DIR/lib/x64/$output_subdir/"
    fi
    
    # Copy static library if exists
    if [ -f "src/cslib/.libs/libcs.a" ]; then
        cp src/cslib/.libs/libcs.a "$OUTPUT_DIR/lib/x64/$output_subdir/"
    fi
