#!/usr/bin/env python3
"""
Generate the FreeTDS cslib (Client Server Library) patch file.
...
def main():
    ...
    patch_file = os.path.join(script_dir, 'freetds-cslib.patch')
...
+# Add cslib (Client Server Library) shared library build if enabled
+if(BUILD_SHARED_LIB)
+    add_subdirectory(src/cslib)
+endif()
+
...
+AC_ARG_ENABLE(cslib,
+	AS_HELP_STRING([--enable-cslib], [build combined shared library libcs.so (Client Server Library)]))
+AM_CONDITIONAL(BUILD_CSLIB, test "$enable_cslib" = "yes")
+
+# Set visibility flags for cslib (hide symbols by default on GCC)
+CSLIB_VISIBILITY_CFLAGS=""
+if test "$ac_compiler_gnu" = "yes"; then
+	CSLIB_VISIBILITY_CFLAGS="-fvisibility=hidden"
+fi
+AC_SUBST(CSLIB_VISIBILITY_CFLAGS)
+
 # ------------------------------------------------------------
...
+DIST_SUBDIRS = utils replacements tds server pool ctlib dblib odbc apps cslib
...
+# If building cslib, we need server even if not explicitly enabled
+if BUILD_CSLIB
+SUBDIRS += server
+endif
 endif
...
+
+# Build cslib (libcs.so) if enabled
+if BUILD_CSLIB
+SUBDIRS += cslib
+endif
...
'''
    ...
    print("  - src/cslib/CMakeLists.txt (new file)")
    print("  - src/cslib/Makefile.am (new file)")
    print("  - src/cslib/freetds_cslib.c (new file)")
